...
function plot_RadarFOV_ExpA(radar_1_pos_3d,radar_2_pos_3d,theta,phi,max_range,beam_angle,z_limits)
...
for radar_ind = [1,2]
    ...
    rotate(center_line, [1 0 0], -phi, [radar_pos(1) radar_pos(2) radar_pos(3)])  % Anticlock-wise rotation by angle phi
    ...
    for R=sqrt(linspace(eps.^2,max_range.^2,50))%generate contours along the top and bottom plane for R~=max_range or generate the whole thing for R=max_range, span with square to make it equally dense for all radii
        ...
        if R==max_range
            ...
            rotate(p0, [1 0 0],-phi,[radar_pos(1) radar_pos(2) radar_pos(3)])% Anticlockwise rotation by -phi
            Z0=p0.ZData;
            ...
            rotate(p0bot, [1 0 0],-phi,[radar_pos(1) radar_pos(2) radar_pos(3)])%rotation phi
            Z0=p0bot.ZData;
            Z0(Z0<z_limits(1))=NaN; % Anything below z_limits(1) is set to NaN 
            Z0(Z0>z_limits(2))=NaN; % Anything above z_limits(2) is set to NaN
